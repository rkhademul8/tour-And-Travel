...
const BookingRefundProcess = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      ...
      .then((data) => {
        const refundFilterData = data?.TotalBookingData.filter((refundData) => {
          return refundData.status === "Refund In Processing";
        });
        refundFilterData?.map((item, index) => (item.serial = index + 1));
        setRefundData(refundFilterData);
        setIsloading(true);
      });
    ...
    //     .then((res) => res.json())
    //     .then((data) => {
    //       const refundFilterData = data.filter((refundData) => {
    //         return refundData.status === "Refund In Processing";
    //       });
    //       refundFilterData?.map((item, index) => (item.serial = index + 1));
    //       setRefundData(refundFilterData);
...
export default BookingRefundProcess;

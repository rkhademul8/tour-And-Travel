...
const BookingVoidProcess = () => {
  const user = secureLocalStorage.getItem("user-info");
  const navigate = useNavigate();
  const [voidOnProcess, setVoidOnProcess] = useState([]);
  const [isLoading, setIsloading] = useState(false);
  // //console.log(voidOnProcess);
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      .then((res) => res.json())
      .then((data) => {
        const voidFilterData = data?.TotalBookingData.filter((refundData) => {
          return refundData.status === "Void In Processing";
        });
        voidFilterData?.map((item, index) => (item.serial = index + 1));
        setVoidOnProcess(voidFilterData);
        setIsloading(true);
      });
    ...
    //     .then((res) => res.json())
    //     .then((data) => {
    //       const voidFilterData = data.filter((refundData) => {
    //         return refundData.status === "Void In Processing";
    //       });
    //       voidFilterData?.map((item, index) => (item.serial = index + 1));
    //       setVoidOnProcess(voidFilterData);
  ...
  return (
    <Box className="DestinaTionWise1">
      <table>
        ...
        {isLoading === true ? (
          <>
            {voidOnProcess.length !== 0 ? (
              <>
                {voidOnProcess.map((data) => (
                  <tr>
                    <>
                      <td>{data?.serial}</td>
                      <td>
...
export default BookingVoidProcess;

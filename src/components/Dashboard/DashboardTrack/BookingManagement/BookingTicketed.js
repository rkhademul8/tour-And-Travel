...
const BookingTicketed = () => {
  const navigate = useNavigate();
  const [ticketedData, setTicketedeData] = useState([]);
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      ...
      .then((data) => {
        const ticketedData = data?.TotalBookingData.filter((data) => {
          return data?.status === "Ticketed";
        });

        ticketedData?.map((item, index) => (item.serial = index + 1));
        setTicketedeData(ticketedData);
        setLoading(true);
      });
  ...
  return (
    <Box className="DestinaTionWise1">
      <table>
        ...
        {isLoading === true ? (
          <>
            {ticketedData.length !== 0 ? (
              <>
                {ticketedData.map((data) => (
                  <tr>
                    <td>{data?.serial}</td>
                    <td>
...
export default BookingTicketed;

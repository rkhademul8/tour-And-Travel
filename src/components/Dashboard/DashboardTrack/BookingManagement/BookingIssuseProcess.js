...
const BookingIssuseProcess = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      .then((res) => res.json())
      .then((data) => {
        const issueProcessFilter = data?.TotalBookingData.filter((data) => {
          return data?.status === "Issue In Processing";
        });
        issueProcessFilter?.map((item, index) => (item.serial = index + 1));
        setIssueData(issueProcessFilter);
        setIsloading(true);
      });
...
export default BookingIssuseProcess;

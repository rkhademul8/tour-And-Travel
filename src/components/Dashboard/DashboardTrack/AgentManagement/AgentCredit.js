...
const AgentCredit = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      .then((res) => res.json())
      .then((data) => {
        const agentRejected = data?.TotalAgentData.filter((data) => {
          return data.credit > "0";
        });
        agentRejected.map((item, index) => (item.serial = index + 1));
        setAgentAllData(agentRejected);
        setIsloading(true);
      });
  }, []);
              ...
              <>
                {agentAllData.map((data) => (
                  <tr>
                    ...
                    <td>
                      {data?.credit > "0" ? (
                        <span style={{ color: "red" }}> {data?.credit}</span>
                      ) : (
                        <span> {commaNumber(Number(data?.credit))}</span>
                      )}
                    </td>
                    <td>{commaNumber(Number(data?.bonus))}</td>
                    <td>

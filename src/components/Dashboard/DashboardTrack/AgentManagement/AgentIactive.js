...
const AgentIactive = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      .then((res) => res.json())
      .then((data) => {
        const agentDeactive = data?.TotalAgentData.filter((data) => {
          return data.status === "deactive";
        });
        agentDeactive.map((item, index) => (item.serial = index + 1));
        setAgentAllData(agentDeactive);
        setIsloading(true);
      });
  }, []);

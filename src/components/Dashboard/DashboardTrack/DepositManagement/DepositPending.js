...
const DepositPending = () => {
  ...
  const [pendingData, setPendinhData] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch("https://api.flyfarint.com/v.1.0.0/Admin/Stats/Dashboard.php")
      ...
      .then((data) => {
        ...
        pendingDataFilter?.map((item, index) => (item.serial = index + 1));
        setPendinhData(pendingDataFilter);
        setIsloading(true);
      });
  }, []);
                    ...
                    <td>
                      {data?.createdAt
                        ? shortEnglishHumanizer(
                            Math.abs(new Date(data?.createdAt) - new Date())
                          )
                        : "Not Found"}
                    </td>

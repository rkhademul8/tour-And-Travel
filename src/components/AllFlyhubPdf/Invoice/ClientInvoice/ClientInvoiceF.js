...
import barCode from "../../../../image/logo/barCode.png";
...
const ClientInvoiceF = ({ allData }) => {
  ...
  useEffect(() => {
    ...
    const url3 = `https://api.flyfarint.com/v.1.0.0/Queues/Booking.php?agentId=${agentID}&search=all`;
    ...
    const fetchUserData3 = fetch(url3)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleToPrint = useReactToPrint({
    ...
    documentTitle: `Client_invoice_${fareCost[0]?.deptFrom}-${fareCost[0]?.arriveTo}_${fareCost[0]?.travelDate}_PAX-${fareCost[0]?.pax}`,
      ...
      {Object.keys(fareCost).length !== 0 ? (
        <>
          <Box className="invoice-btn">
            <button onClick={handleToPrint}>Client Invoice</button>
          </Box>

          <Box mt={8} style={{ display: "none" }}>
            <Container>

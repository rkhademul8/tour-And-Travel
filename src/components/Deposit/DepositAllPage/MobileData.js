...
const MobileData = () => {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((res) => res.json())
      .then((data) => {
        const mobileDataFilter = data.filter(
          (data) => data?.paymentway === "Mobile"
        );
        console.log(mobileDataFilter);

        const count = mobileDataFilter.length;
        const pageNumber = Math.ceil(count / size);
        setPageCount(pageNumber);
        setPagiNation(mobileDataFilter);
        setDepositData(data);
      });
  });
  return (

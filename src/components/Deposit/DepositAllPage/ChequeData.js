...
const ChequeData = () => {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((res) => res.json())
      .then((data) => {
        const ChequeDataFilter = data.filter(
          (data) => data?.paymentway === "Cheque"
        );
        console.log(ChequeDataFilter);

        const count = ChequeDataFilter.length;
        const pageNumber = Math.ceil(count / size);
        setPageCount(pageNumber);
        setPagiNation(ChequeDataFilter);
        setDepositData(data);
      });
  });
  return (

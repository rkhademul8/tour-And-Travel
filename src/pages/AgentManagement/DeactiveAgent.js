    ...
    fetch(
      ...
      .then((data) => {
        fetch(
          ...
          .then((res) => res.json())
          .then((data) => {
            const deactiveteData = data.filter((filterDeactivateData) => {
              return filterDeactivateData?.status === "deactivate";
            });
            const count = deactiveteData.length;
            const pageNumber = Math.ceil(count / size);
            setPageCount(pageNumber);
            setSearchData(deactiveteData);
            setSubAgentData(deactiveteData);
            setIsloading(true);
          });
      });
  }, [agentId]);

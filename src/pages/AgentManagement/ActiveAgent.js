    ...
    fetch(
      ...
      .then((data) => {
        fetch(
          ...
          .then((res) => res.json())
          .then((data) => {
            const activeData = data.filter((filterActiveData) => {
              return filterActiveData?.status === "active";
            });
            const count = activeData.length;
            const pageNumber = Math.ceil(count / size);
            setPageCount(pageNumber);
            setSearchData(activeData);
            setSubAgentData(activeData);
            setIsloading(true);
          });
      });
  }, []);
                  ...
                  <td>
                    {data?.status === "active" ? (
                      <button
                        ...
                        onClick={() => handleDeactivate(data?.subagentId, data?.agentId)}
                      >
                        Deactivate
                      </button>
                    ) : data?.status === "reject" ? (
                      ...
                      <Box>
                        <IconButton
                          sx={{ color: "#0E8749" }}
                          onClick={() => handleActive(data?.subagentId, data?.agentId)}
                        >
                          <CheckCircleIcon />
                        </IconButton>
                        <IconButton
                          sx={{ color: "var(--remove-color)" }}
                          onClick={() => handleReject(data?.subagentId, data?.agentId)}
                        >
                          <CancelIcon />
                        </IconButton>
                      </Box>
